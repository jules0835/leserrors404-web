...
export default function OtpConfig() {
  ...
  const { data: otpDetails, isLoading } = useQuery({
    queryKey: ["otpDetails"],
    queryFn: async () => {
      const { data } = await axios.get("/api/user/dashboard/security/otp")
  ...
  const generateOtpKeyMutation = useMutation({
    mutationFn: async () => {
      const { data } = await axios.post("/api/user/dashboard/security/otp")
  ...
  const verifyOtpMutation = useMutation({
    mutationFn: async (token) => {
      const { data } = await axios.put("/api/user/dashboard/security/otp", {
        token,
      })
  ...
  const disableOtpMutation = useMutation({
    mutationFn: async () => {
      const { data } = await axios.delete("/api/user/dashboard/security/otp")
  ...
  return (
    <div className="p-4 border rounded-md">
      <h3 className="text-md font-bold mb-4">{t("title")}</h3>
      <div className="space-y-4">
        ...
        {generateOtpKeyMutation.isSuccess && otpOpen && (
          <div className="space-y-4 text-center justify-center flex flex-col">
